  ...
  if (!href) {
    return (
      ...
      >
        ...
        <div
          style={{ fontWeight: 600, fontSize: "0.85rem", marginBottom: "3px" }}
        >
          {label}
        </div>
...
export default function EventDetailPage({
  ...
  const hasPeople =
    hasTopics || event.videoPresenter?.length > 0 || event.anchor;
  const hasVolunteers = event.volunteers?.length > 0;
  const hasAcknowledgements =
    event.acknowledgements?.length > 0 || event.closingNote;
  const tabs = [
        ...
        <div className="container" style={{ position: "relative", zIndex: 1 }}>
          ...
          >
            ...
            >
              ...
              >
                <DynamicIcon
                  name={status === "completed" ? "CheckCircle" : "Calendar"}
                  size={12}
                />{" "}
        ...
        >
          ...
          {activeTab === "volunteers" && event.volunteers?.length > 0 && (
            <section>
              <SectionHeader
                icon="Zap"
                title="Volunteers — The Unsung Heroes"
                color={color}
              />
              <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
